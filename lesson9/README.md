...
```bash
find logs/ -maxdepth 1 -type f -exec du -b {} + | sort -rn | head -1 | cut -f2 > biglog.txt
```
...
```bash
grep -i "host" config/*.conf > host_params.txt
```
...
```bash
zip project_backup.zip \
  config/*.conf \
  logs/*.log \
  logs/old/*.log \
  errors.txt 2>/dev/null
```
...
```bash
wc -l config/*.conf | head -n -1 > conf_stats.txt
```
